<template>
  <div class="home">
    ...
    <!-- //? Loading Animation -->
    <LoadingOverlay v-if="$fetchState.pending" />
      ...
      <div v-if="searchInput !== ''" class="container movies">
        <div id="movie-grid" class="movies-grid">
          <div v-for="(movie, index) in searchedMovies" :key="index" class="movie">
            ...
            <div class="info">
              <p class="title">{{ movie.title.slice(0, 27) }} <span v-if="movie.title.length > 27">...</span></p>
              <p class="release">
                Released:
                {{
  ...
  methods: {
    async getMovies() {
      const data = axios.get(
        `https://api.themoviedb.org/3/movie/now_playing?api_key=ccf177dce433bf5ecc28ab7f9bfe847e&language=en-US&page=1`
      )
    ...
    async searchMovies() {
      const data = axios.get(
        `https://api.themoviedb.org/3/search/movie?api_key=ccf177dce433bf5ecc28ab7f9bfe847e&language=en-US&page=1&query=${this.searchInput}`
      )
      const result = await data
      result.data.results.forEach((movie) => {
